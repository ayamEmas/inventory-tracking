                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div x-data="{ show: false }" 
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600 mt-1">Showing users from {{ auth()->user()->department->name ?? 'your department' }} only</p>
                            @endif
                        </div>
                        @if($hasFullAccess || auth()->user()->role === 'HOD')
                    ...
                    <div x-data="{ show: false }" 
                        ...
                        @php
                            ...
                            $groupedUsers = $groupedUsers->sortBy(function ($users, $role) use ($roleHierarchy) {
                        ...
                        @forelse ($groupedUsers as $role => $roleUsers)
                            <!-- Role Section Header -->
                            <div class="mb-4">
                                <h3 class="text-lg font-semibold text-gray-800 bg-gray-100 px-4 py-3 rounded-lg border-l-4 border-indigo-500 text-center">
                                    {{ $role }} ({{ $roleUsers->count() }} {{ $roleUsers->count() === 1 ? 'user' : 'users' }})
                                </h3>
                            </div>
                                            ...
                                            <tr class="hover:bg-gray-50 transition-colors duration-300">
                                                ...
                                                <td class="px-6 py-4 text-sm text-gray-600 text-center">
                                                    <div class="flex justify-center space-x-3">
                                                        ...
                                                        @if($hasFullAccess || 
                                                            (auth()->user()->role === 'HOD' && auth()->user()->department_id === $user->department_id))
                    ...
                    <div x-data="{ show: false }" 
                        ...
                        @forelse ($groupedUsers as $role => $roleUsers)
                            <!-- Role Section Header for Mobile -->
                            <div class="mb-4">
                                <h3 class="text-lg font-semibold text-gray-800 bg-gray-100 px-4 py-3 rounded-lg border-l-4 border-indigo-500 text-center">
                                    {{ $role }} ({{ $roleUsers->count() }} {{ $roleUsers->count() === 1 ? 'user' : 'users' }})
                                </h3>
